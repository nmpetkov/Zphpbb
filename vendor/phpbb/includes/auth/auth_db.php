...
function login_db($username, $password, $ip = '', $browser = '', $forwarded_for = '')
{
	...
	// Check password ...
    // Zphpbb >
    if (zikula_check_hash($password, $username))
...
* Zphpbb function
*/
function zikula_check_hash($password, $username)
{
    if (class_exists('SecurityUtil')) {
        $hashedPassword = UserUtil::getVar('pass', UserUtil::getIdFromName($username));
        $passMatch = SecurityUtil::checkSaltedHash($password, $hashedPassword);
    } else {
        require_once __DIR__.'/../../../../lib/ztools.php';
        $userdata = Ztools::ZikulaUserData(false, $username);
        $passMatch = Ztools::passwordsMatch($password, $userdata['pass']);
    }
