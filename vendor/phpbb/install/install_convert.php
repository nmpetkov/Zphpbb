...
class install_convert extends module
{
	...
	*/
	function get_convert_settings($sub)
	{
        ...
        // Zphpbb: below replaced '. with $phpbb_root_path.'/install
		if (!file_exists($phpbb_root_path.'/install/convertors/convert_' . $convertor_tag . '.' . $phpEx))
		{
			$this->p_master->error($lang['CONVERT_NOT_EXIST'], __LINE__, __FILE__);
		}

        // Zphpbb: below replaced '. with $phpbb_root_path.'/install
		include($phpbb_root_path.'/install/convertors/convert_' . $convertor_tag . '.' . $phpEx);
		...
		$src_dbport			= $dbport;
		$src_dbuser			= $dbuser;
		//$src_dbpasswd		= $dbpasswd; // they unset $dbpasswd, so use our
        $src_dbpasswd		= $GLOBALS['ZConfig']['DBInfo']['databases']['default']['password'];
		$src_dbname			= $dbname;
        global $phpbb2_tableprefix;
		$src_table_prefix	= $phpbb2_tableprefix;
		$forum_path			= 'modules/ZphpBB2/vendor/phpBB2';
		$refresh			= 1;
        // Zphpbb <
...
//					'S_HIDDEN'	=> $s_hidden_fields,
	...
	*/
	function convert_data($sub)
	{
		...
		require($phpbb_root_path . 'config.' . $phpEx);
		require($phpbb_root_path . 'includes/constants.' . $phpEx);
		//require($phpbb_root_path . 'includes/db/' . $dbms . '.' . $phpEx); // Zphpbb - already included
		//require($phpbb_root_path . 'includes/functions_convert.' . $phpEx); // Zphpbb - already included
		...
		if (!file_exists($phpbb_root_path . 'install/convertors/convert_' . $convert->convertor_tag . '.' . $phpEx))
        // Zphpbb <
		{
		...
		if (file_exists($phpbb_root_path . 'install/convertors/functions_' . $convert->convertor_tag . '.' . $phpEx))
		{
			include($phpbb_root_path . 'install/convertors/functions_' . $convert->convertor_tag . '.' . $phpEx);
		}
        // Zphpbb <
		...
		include($phpbb_root_path . 'install/convertors/convert_' . $convert->convertor_tag . '.' . $phpEx);
        // Zphpbb <
		...
		if (!$current_table && !$skip_rows)
		{
            ...
            LogUtil::registerStatus($user->lang['CONFIG_CONVERT'].': '.$user->lang['DONE']);
			/*$template->assign_block_vars('checks', array(
		...
		while ($current_table < sizeof($convert->convertor['schema']))
		{
            ...
            LogUtil::registerStatus($user->lang['FILLING_TABLE'] . $schema['target']);
			/*$template->assign_block_vars('checks', array(
...
//				$percentage = ($skip_rows == 0) ? 0 : floor(100 / ($total_rows / $skip_rows));
        ...
        LogUtil::registerStatus($user->lang['FILLING_TABLES'].': '.$user->lang['DONE']);
	...
	*/
	function sync_forums($sync_batch)
	{
        ...
        LogUtil::registerStatus($user->lang['SYNC_TOPICS']);
		/*$template->assign_block_vars('checks', array(
			'S_LEGEND'	=> true,
		...
		while ($sync_batch <= $primary_max && still_on_time())
		{
            ...
            LogUtil::registerStatus(sprintf($user->lang['SYNC_TOPIC_ID'], $sync_batch, ($sync_batch + $batch_size)) . ((defined('DEBUG_EXTRA') && function_exists('memory_get_usage')) ? ' [' . ceil(memory_get_usage()/1024) . ' ' . $user->lang['KIB'] . ']' : '') .': '.$user->lang['DONE']);
			/*$template->assign_block_vars('checks', array(
...
// Zphpbb >
// dummy class install_convert to extend from
class module
{
	var $id = 0;
	var $type = 'install';
	...
	*/
	function error($error, $line = 0, $file = '', $skip = false)
	{
		...
		//exit_handler();
        //LogUtil::registerError($content);
        throw new Zikula_Exception_Fatal($content, 0, null);
	}
}
// Zphpbb <
